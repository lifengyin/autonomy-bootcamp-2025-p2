...
def read_queue(
    ...
    """
    # Read from worker's output queue and print it using logger
    while not controller.is_exit_requested():
        try:
            status = status_queue.queue.get(timeout=HEARTBEAT_PERIOD + ERROR_TOLERANCE)
...
def main() -> int:
    ...
    # Create your queues
    status_queue = queue_proxy_wrapper.QueueProxyWrapper(mp_manager, 10)
    ...
    threading.Thread(
        target=read_queue,
        args=(status_queue, controller, main_logger),
        daemon=True,
    ).start()
