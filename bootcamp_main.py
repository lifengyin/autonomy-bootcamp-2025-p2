...
HEARTBEAT_TO_MAIN_QUEUE_MAX_SIZE = 10
...
MAIN_RUNTIME_SECONDS = 100
MAIN_QUEUE_TIMEOUT_SECONDS = 0.1
...
def main() -> int:
    ...
    result, heartbeat_sender_manager = worker_manager.WorkerManager.create(
        worker_properties=heartbeat_sender_properties, local_logger=main_logger
    )
    if not result:
    ...
    result, heartbeat_receiver_manager = worker_manager.WorkerManager.create(
        worker_properties=heartbeat_receiver_properties, local_logger=main_logger
    )
    if not result:
